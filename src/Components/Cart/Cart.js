    ...
    return (
        <Container className={`${classes.mycart} position-fixed`}>
            ...
            {cartCtx.items.map((item) => {
                return <CartItem key={item.identity} title={item.title} imageUrl={item.imageUrl} price={item.price} quantity={item.quantity} identity={item.identity} database_id={item.database_id} />
            })}
            <Row className='my-2'>
